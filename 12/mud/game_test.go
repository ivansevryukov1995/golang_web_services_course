...
func TestGameMiltiplayer(t *testing.T) {
	for caseNum, commands := range game1Cases {
		...
		// Одна горутина на игрока, живут весь тест
		for name, p := range players {
			go func(name string, p *Player) {
				for msg := range p.GetOutput() {
					mu.Lock()
					lastOutput[name] = msg
					mu.Unlock()
					wg.Done()
		...
		for _, item := range commands {
			...
			// Считаем сколько сообщений ожидаем
			wg.Add(len(item.answers))
			players[item.player].HandleInput(item.command)

			// Ждём пока горутины запишут ответы
			wg.Wait()

			// Читаем под локом
			mu.Lock()
			got := make(map[string]string, len(lastOutput))
			for k, v := range lastOutput {
				got[k] = v
			}
			mu.Unlock()

			if !reflect.DeepEqual(got, item.answers) {
				t.Error("case:", caseNum, item.step,
					"\n\tcmd:", item.command,
					"\n\tresult:  ", got,
